...
public class MecanumDrive {
    ...
    public void drive(double x, double y, double rotation, double knobAngle) {
        ...
        case KNOB_FIELD:
            ...
            /*
             * if(!rotationLatch && Math.abs(error) > rotationLatchDeadband) { error = 0; }
    ...
    // public boolean driveToPos(Vector2d targetPos, double orientation, double
    // speed) {
    // return driveToPos(targetPos, orientation, speed, STOP_DISTANCE);
    // }

    // public boolean driveToPos(Vector2d targetPos, double orientation, double
    // speed, double stopDistance) {
    // Vector2d position = IO.ofs.getPos();
    //
    // //Vector2d direction1 = new Vector2d(SPEED * Math.cos(Math.toRadians(ANGLE)),
    ...
    // xOutput = xyRatio * direction.getX() / Math.abs(direction.getY()) *
    // Math.min(1, dirMag);
    // yOutput = direction.getY() / Math.abs(direction.getY()) * Math.min(1,
    // dirMag);
    // } else {
    // xOutput = direction.getX() / Math.abs(direction.getX()) * Math.min(1,
    // dirMag);
    // yOutput = direction.getY() / Math.abs(direction.getX()) / xyRatio *
    // Math.min(1, dirMag);
    // }
    //
    ...
    public enum DriveType {
        KNOB_FIELD, STICK_FIELD, ROTATE_PID, STICK_FIELD2;
    }
}