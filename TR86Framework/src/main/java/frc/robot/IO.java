...
//Having them all in one class makes it easier to initialize them as well as makes it easier to read other subsystems.
...
//must have kuaiLabs vendor package installed
...
public class IO {
    // These are placeholder ports, replace them with the actual port in the object
    ...
    public static Talon subsysTalon = new Talon(examplePort);
    public static Talon stateSubsysTalon = new Talon(examplePort);

    // The NavX is the gyro sensor of the robot
    public static NavX exampleNavX = new NavX();

    // The Compressor and Relay are used in conjunction with all solenoids on the
    // robot
    ...
    // Solenoids are pneumatic activators (require a module and a port; can be
    // inverted)
